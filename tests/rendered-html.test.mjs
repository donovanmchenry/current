...
test("connects the lesson shell to a functional learning map", async () => {
  ...
  const generateLesson = await readFile(new URL("../app/api/lessons/generate/route.ts", import.meta.url), "utf8");
  const generatePath = await readFile(new URL("../app/api/paths/generate/route.ts", import.meta.url), "utf8");
  const modelRouting = await readFile(new URL("../lib/model-routing.ts", import.meta.url), "utf8");
  ...
  assert.match(workspace, /onResetDemo=\{resetDemo\}/);
  ...
  assert.match(workspace, /await clearSourceArtifacts\(\)/);
  assert.match(workspace, /setActivePathId\("long-running"\)[\s\S]*setActiveConceptIndex\(1\)[\s\S]*resetLessonActivity\("read"\)/);
  ...
  assert.match(workspace, /className="concept-row"[\s\S]*disabled=\{conceptLocked\}[\s\S]*onClick=\{\(\) => openLesson\(activePath\.id, index\)\}/);
  ...
  assert.match(map, /aria-label="Confirm demo reset"/);
  assert.match(map, /window\.sessionStorage\.removeItem\(sourceAgentSessionKey\)/);
  ...
  assert.match(map, /const previousVersionRef = useRef\(version\)/);
  assert.match(map, /previousVersionRef\.current === version/);
  assert.match(map, /duration: 0/);
  assert.doesNotMatch(map, /\n\s*refit\(\);\n\s*window\.addEventListener\("resize", refit\)/);
  ...
  assert.match(map, /disabled=\{inspectedConceptState === "upcoming"\}/);
  assert.match(map, /inspectedConceptState === "upcoming" \? "Complete prior concepts to open" : "Start"/);
  ...
  assert.match(workspace, /Authored by \$\{currentModelLabel\(lesson\.model\)\}/);
  assert.match(workspace, /Evaluated by \$\{currentModelLabel\(props\.evaluation\.model\)\}/);
  assert.match(sourceArtifacts, /indexedDB\.open\(databaseName, 1\)/);
  assert.match(sourceArtifacts, /store\.put\(/);
  assert.match(sourceArtifacts, /transaction\.objectStore\(storeName\)\.clear\(\)/);
  ...
  assert.match(modelRouting, /pathPlanner:[\s\S]*model: "gpt-5\.6-sol"[\s\S]*reasoningEffort: "medium"/);
  assert.match(modelRouting, /lessonAuthor:[\s\S]*model: "gpt-5\.6-terra"[\s\S]*reasoningEffort: "medium"/);
  assert.match(modelRouting, /learningCoach:[\s\S]*model: "gpt-5\.6-luna"[\s\S]*reasoningEffort: "low"/);
  assert.match(modelRouting, /sourceResearch:[\s\S]*model: "gpt-5\.6-sol"[\s\S]*reasoningEffort: "high"/);
  assert.match(generatePath, /currentModelRoutes\.pathPlanner/);
  assert.match(generateLesson, /currentModelRoutes\.lessonAuthor/);
  assert.match(coach, /currentModelRoutes\.learningCoach/);
  assert.match(coach, /store: false/);
  assert.match(generatePath, /signal: AbortSignal\.timeout\(30_000\)/);
  assert.match(generatePath, /max_output_tokens: 8000/);
  assert.match(refreshSource, /currentModelRoutes\.sourceResearch/);
  assert.match(refreshSource, /reasoning: \{ effort: route\.reasoningEffort \}/);
  ...
  assert.match(styles, /\.concept-row:disabled[^}]*cursor:\s*not-allowed/s);
  assert.match(styles, /\.rail-primary-action:disabled[^}]*cursor:\s*not-allowed/s);
  assert.match(styles, /\.lesson-scroll[^}]*overflow-x:\s*hidden/s);
  assert.match(styles, /@media \(max-width: 720px\)[\s\S]*\.lesson-module[^}]*width:\s*min\(720px, calc\(100% - 30px\)\)/s);
  ...
  assert.match(styles, /\.map-reset-trigger[^}]*background:\s*transparent/s);
  ...
  assert.match(styles, /\.generation-provenance[^}]*color:\s*var\(--text-muted\)/s);
