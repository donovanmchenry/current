...
- **Current Classroom.** A dedicated educator surface at `/classroom` lets teachers create rostered classes, assign any Current path with a due date, launch a student-labeled assignment session, receive its recall evidence live, turn shared misconceptions into targeted review, and approve curriculum updates before they reach the class.
...
The current suite contains 30 passing tests. `npm test` builds the production application before running endpoint, scheduler, persistence, Classroom, and rendered-behavior checks.
